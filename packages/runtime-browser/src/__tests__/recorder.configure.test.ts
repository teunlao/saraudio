...
describe('Recorder.configure', () => {
  ...
  it('supports asynchronous stage loaders that return controllers', async () => {
    const pipeline = new Pipeline({
      now: () => 0,
      createId: () => 'id',
    });
    ...
    const loader = async () => controller;

    const recorder = createRecorder({ runtime, stages: [loader], segmenter: false });

    await recorder.configure();
    expect(created).toBe(1);
    expect(configured).toBe(1);

    await recorder.configure({ stages: [loader], segmenter: false });
    expect(created).toBe(1);
    expect(configured).toBe(2);
