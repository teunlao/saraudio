...
const handleToggleSegment = (s: Segment) => {
  if (playingId.value === s.id) stopCurrent();
  else playSegment(s);
};
  ...
  <div class='min-h-screen bg-gray-900 text-white p-8'>
    <div class='max-w-6xl mx-auto'>
      <header class='mb-8'>
        ...
        <p class='text-gray-400'>Full-featured audio recorder with VAD, device selection, and multiple export formats</p>
      </header>
      ...
      <section class='mb-8 p-6 bg-gray-800 rounded-lg space-y-4'>
        ...
        <div class='flex gap-4 items-end'>
          <div class='flex-1'>
            ...
            <select v-model='audioInputs.selectedDeviceId.value' :disabled='audioInputs.enumerating.value || isRunning'
          ...
          <button type='button' @click='audioInputs.refresh' :disabled='audioInputs.enumerating.value || isRunning'
        ...
        <div class='grid grid-cols-2 gap-4'>
          <div>
            <label class='block text-sm text-gray-400 mb-2'>Energy Threshold: {{ thresholdDb }} dB</label>
      ...
      <div class='flex gap-4 mb-8'>
        <button type='button' @click='handleStart' :disabled='isRunning || rec.status.value === "stopping"'
        ...
        <button type='button' @click='handleStop' :disabled='!isRunning'
        ...
        <button type='button' @click='rec.clearSegments' :disabled='rec.segments.value.length === 0'
      ...
      <div class='mb-8 space-y-2'>
        <div class='px-4 py-2 rounded-lg'
          :class='{ "bg-green-900/50 border border-green-500": rec.status.value === "running", "bg-blue-900/50 border border-blue-500": rec.status.value === "acquiring", "bg-gray-800 border border-gray-600": rec.status.value === "idle" }'>
          <span class='text-sm text-gray-400'>Status:</span>
          <span class='ml-2 font-mono'>{{ rec.status.value }}</span>
        </div>
        <div v-if='audioInputs.error.value' class='px-4 py-2 bg-red-900/50 border border-red-500 rounded-lg'>
          <span class='text-sm text-red-400'>Device Error:</span>
          <span class='ml-2'>{{ audioInputs.error.value }}</span>
        </div>
        <div v-if='rec.error.value' class='px-4 py-2 bg-red-900/50 border border-red-500 rounded-lg'>
      ...
      <div class='grid grid-cols-2 gap-8 mb-8'>
        <div class='p-6 bg-gray-800 rounded-lg'>
          ...
          <div class='flex items-center gap-4 mb-4'>
            <div :class="['w-6 h-6 rounded-full transition-all', rec.vad.value?.speech ? 'bg-green-500' : 'bg-gray-600']" />
            <div class='font-mono text-lg'>{{ rec.vad.value?.speech ? 'Speech' : 'Silence' }}</div>
          </div>
          <div class='text-sm text-gray-400'>Score: {{ rec.vad.value?.score.toFixed(2) ?? '0.00' }}</div>
        </div>
        <div class='p-6 bg-gray-800 rounded-lg'>
          <h2 class='text-xl font-semibold mb-4'>Audio Levels</h2>
          <div class='space-y-3'>
            <div>
              <div class='flex justify-between text-sm text-gray-400 mb-1'><span>RMS</span><span>{{ meterLevels.rms.value.toFixed(3) }}</span></div>
              <div class='w-full bg-gray-700 h-3 rounded overflow-hidden'>
                <div class='bg-blue-500 h-full transition-all' :style='`width: ${Math.min(meterLevels.rms.value * 100, 100)}%`' />
              </div>
            </div>
            <div>
              <div class='flex justify-between text-sm text-gray-400 mb-1'><span>Peak</span><span>{{ meterLevels.peak.value.toFixed(3) }}</span></div>
              <div class='w-full bg-gray-700 h-3 rounded overflow-hidden'>
                <div class='bg-green-500 h-full transition-all' :style='`width: ${Math.min(meterLevels.peak.value * 100, 100)}%`' />
              </div>
            </div>
            <div class='flex justify-between text-sm text-gray-400'><span>dB</span><span>{{ meterLevels.db.value === -Infinity ? '-∞' : meterLevels.db.value.toFixed(1) }}</span></div>
          </div>
        </div>
      </div>

      <section class='mb-8 p-6 bg-gray-800 rounded-lg'>
        <h2 class='text-xl font-semibold mb-4'>Captured Segments ({{ rec.segments.value.length }})</h2>
        <div v-if='rec.segments.value.length === 0' class='text-gray-500 text-center py-8'>Talk into the microphone to capture segments</div>
        <div v-else class='space-y-2 max-h-96 overflow-y-auto'>
          <div v-for='segment in rec.segments.value' :key='segment.id' class='p-4 bg-gray-700 rounded-lg flex items-center gap-4'>
            <button type='button' @click='handleToggleSegment(segment)'
            ...
            <div class='flex-1'>
              <div class='flex items-center justify-between mb-1'>
                <div class='font-mono text-xs text-gray-400'>#{{ segment.id.slice(0, 8).toUpperCase() }}</div>
                <div class='text-xs text-gray-400'>{{ (segment.durationMs / 1000).toFixed(2) }}s</div>
              </div>
              <div class='text-sm text-gray-300'>{{ new Date(segment.startMs).toLocaleTimeString() }} → {{ new Date(segment.endMs).toLocaleTimeString() }}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer class='mt-8 text-sm text-gray-500 text-center'>
        <p>Microphone requires HTTPS (or localhost) and user permission. Adjust thresholds to tune sensitivity.</p>
      </footer>
    </div>
  </div>
</template>
